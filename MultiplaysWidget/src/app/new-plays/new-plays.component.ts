...
export class NewPlaysComponent extends PlaysViewComponent<Result> {
  ...
  protected processData(plays: Result) {
    if (plays) this.refreshChart(plays.plays);
  }
    ...
    for (const geek of data.geeks) {
      ...
      for (const play of plays) {
        ...
        if (play.year >= 1996) {
          ...
          const game = gamesIndex[play.game.bggid];
          firstPlays.push({
            count: playedByThisGeek.length,
            gameName: game.name,
      ...
      embed(this.target!.nativeElement, spec, { actions: true }).then(() => console.log("done"));
    }
  }
}
