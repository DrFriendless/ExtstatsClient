...
type Subdomain = 'Abstract Games' | 'cgs' | 'Unknown' | 'Children\'s Games' | 'Family Games' | 'Party Games' | 'Strategy Games' |
  'Thematic Games' | 'Wargames' | 'Customizable Games';
const FLORENCE_COLOURS: Record<Subdomain, string> = { 'Abstract Games': '#20b020', 'cgs': '#d060d0', 'Unknown': '#aaaaaa',
...
export class FlorenceNightingaleComponent extends PlaysViewComponent<Result> implements OnInit {
  ...
  private playsByGame: Partial<Record<string, number>> = {};
  private playsBySubdomain: Partial<Record<Subdomain, number>> = {};
  private gi: Record<string, GameData> = {};
  ...
  protected processData(d: Result) {
    console.log(`florence ${!!d} ${!!(d && d.plays)} ${!!(d && d.plays && d.plays.games)} ${!!(d && d.plays && d.plays.plays)}`);
  ...
  private recalc() {
    ...
    for (const bggid in this.playsByGame) {
      mostPlayedGames.push({ bggid: parseInt(bggid), count: this.playsByGame[bggid] || 0, subdomain: this.gi[bggid].subdomain });
    }
    ...
    for (const key in this.playsBySubdomain) {
      const count = this.playsBySubdomain[key as Subdomain] || 0;
      chartData.push({
        name: key,  radius: Math.sqrt(count),
        startAngle: start + padding, endAngle: start + perSubdomain - padding,
        colour: FLORENCE_COLOURS[key as Subdomain] || "#000000", "tooltip": `${key} (${count})`, zindex: 0
      });
      let total = 0;
      const sub: ChartData[] = [];
      ...
      sub.reverse();
      let zindex = 0;
      let col = FLORENCE_COLOURS[key as Subdomain];
